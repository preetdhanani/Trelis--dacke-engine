...
| **Version** | 1.0 |
| **Status** | v1.5 — S1 + S2 validated; M0–M3 built, verified end-to-end (M3 charts native + editable) |
...
| **D8** | **Diagrams as editable native shapes** via deterministic layout of an LLM node/edge spec. *(spike-gated: S5)* | Editable native `.pptx` is a hard product requirement; Mermaid→image yields a flat, non-editable picture. Native-shape composition + our own layout logic is the only editable path. | If S5 fails: v1 falls back to **rendered image + `non-editable` flag**; editable diagrams → v2. |
...
### 5.7 Diagram Builder  *(spike-gated: S5)*
- **Responsibility:** from plain text, build a **process/flow** diagram as editable native shapes.
- **In:** plain-text process description. **Out:** shape+connector placement for the renderer.
- **Core logic:** LLM emits a **node/edge** spec (structured output) from the text (user never writes node/edge syntax); a **deterministic layout** function positions nodes (linear or single-branch) and draws `MSO_SHAPE` boxes + connectors, theme-colored.
- **Failure mode / fallback:** too complex for the layout algorithm → render to image + `non-editable` flag (v1), or flag for human. Editable complex diagrams → v2.
...
| **M4** | Exhibits: diagrams | Diagram Builder (5.7) — one process-flow type (S5), with image-fallback wired. |
...
- **Diagram too complex** → image fallback + `non-editable` flag (or human flag).
...
3. **Is the diagram image-fallback acceptable** for v1 if S5 is costly, or must diagrams be editable-or-deferred? (Note: this template has no dedicated diagram layout — the `image_only` layout's `IM_IMAGEONLY_HERO` slot is the natural target for a diagram, native or image-fallback.)
