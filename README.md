...
> **Status:** M0–M3 built and verified end-to-end (Anthropic, Gemini, and local
> Ollama). Diagrams, QA/overflow, the revision engine, and multi-tenant
...
deck_engine/
  ...
  renderer.py          5.8 seed-slide duplication + fill + native charts
...
66 tests, no external services required (all LLM/network calls are mocked).
...
| M4 Diagram Builder (process-flow diagrams) | ⏳ next |
| M5 QA / overflow checker | ⏳ planned |
| M6 Revision engine (field edit / regen) | ⏳ planned |
| M7 Second tenant (prove tenant-agnosticism) | ⏳ planned |

### Known limitations (today)

- **Diagrams aren't built yet (M4).** A slide the planner marks as needing a
  process-flow diagram renders a flagged placeholder — gracefully, never a crash.
